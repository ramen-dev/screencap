...
// - doneCallback  [optional] - default null - a callback to use 
...
// - jqueryUrl     [optional] - default "https://ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js" - the URL for jQuery to do viewport sizing hack
//
...
function pickupNamedArguments() {
    ...
    if(args.doneCallback)  { doneCallback = args.doneCallback; }
}
...
function doRender() {
    clearInterval(doneCallbackInterval);
...
function waitForCallback() {
  if (doneCallback != null) {
    checkDoneCallback();
    doneCallbackInterval = setInterval(checkDoneCallback, 50);
  }
}

function checkDoneCallback() {
  if (page.evaluate("function() { return " + doneCallback + "; }")) {
    doRender();
  }
}
  ...
  cutoffExecution();
   page.open(args.url, function(status) {
        if(status !== 'success') {
            ...
            if (page.evaluate(function() { return !!jQuery })) {
                jQcb();
            } else {
                nc = true;
                page.includeJs(
...
args.jqueryUrl = args.jqueryUrl || "https://ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js";
